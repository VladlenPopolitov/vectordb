...
sub init_database {
    my ($self,$dbh) = @_;
    if(defined($self->{user}) && defined($self->{password}) && defined($self->{dbname}) ) {
       $dbh->do("CREATE EXTENSION IF NOT EXISTS imgsmlr");
...
sub init_table {
    ...
    if(defined($self->{user}) && defined($self->{password}) && defined($self->{dbname}) ) {
        ...
        $dbh->do("CREATE TABLE public.$table (id int, embedding signature)");
...
sub insert_from_data {
    ...
    if(defined($self->{user}) && defined($self->{password}) && defined($self->{dbname}) ) {
        ...
        for(my $record=0;$record<$totalLines;++$record){
            my $line=$data->getline_format2($record) ; # {pdlref}->($widthFirst:$widthLast,($record)); 
            $line=~ s/\[|\]/ /g;
            $line.=$self->{pad};
...
sub create_index {
    ...
    if(defined($self->{user}) && defined($self->{password}) && defined($self->{dbname}) ) {
        ...
        if($self->{distancetype} eq 'a') { # angular 
         my $sth=$dbh->do("CREATE INDEX ${table}_embeded_idx ON public.$table USING gist (embedding)");
...
sub query {
    ...
    if($distancetype eq "a") {
        $query = 'SELECT id FROM '.$table.' ORDER BY embedding <-> signature_in($1) LIMIT '.$count;
